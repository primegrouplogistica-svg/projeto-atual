...
const UserManagement: React.FC<UserMgmtProps> = ({ users, onSaveUser, onBack }) => {
  ...
  const disabledSalvar = isSaving || !nome || !senha;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm italic">
            Defina perfis e permissões de acesso
          </p>
        </div>
        <div className="flex gap-2">
          <button
            ...
            className={`px-6 py-2 rounded-xl font-bold transition-all shadow-lg ${
              showForm
                ...
                : 'bg-blue-700 text-white shadow-blue-900/20'
            }`}
          >
          ...
          <button
            onClick={onBack}
            className="bg-slate-800 hover:bg-slate-700 px-6 py-2 rounded-xl font-bold border border-slate-700 transition-colors"
          >
            Voltar
          </button>
        </div>
              ...
              <div className="space-y-6 animate-slideDown">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  <h3 className="text-xs font-black text-blue-500 uppercase tracking-[0.2em]">
                    Permissões do Admin Alternativo
                  </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {PERMISSION_GROUPS.map((group, gIdx) => (
                    <div
                      key={gIdx}
                      className="space-y-3 bg-slate-950/40 p-4 rounded-2xl border border-slate-900"
                    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {users
          ...
          .map(u => (
            <Card
              key={u.id}
              className={`border-l-4 group hover:bg-slate-900/70 transition-all ${
                u.ativo ? 'border-l-blue-600' : 'border-l-red-900'
              }`}
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="font-black text-slate-100 group-hover:text-blue-400 transition-colors uppercase tracking-tight">
                    {u.nome}
                  </div>
                  <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">
                    {u.email || 'Sem email cadastrado'}
                  </div>
                </div>
              ...
              {u.perfil === UserRole.CUSTOM_ADMIN && (
                <div className="mb-4">
                  <div className="text-[9px] text-slate-600 font-black uppercase mb-1">
                    Acessos: {u.permissoes?.length || 0} módulos
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {u.permissoes?.slice(0, 3).map(p => (
                      <span
                        key={p}
                        className="text-[8px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded-md font-bold uppercase"
                      >
                        {PERMISSION_GROUPS.flatMap(g => g.perms).find(x => x.id === p)?.label.split(
                          ' ',
                        )[0] || p}
                      </span>
                    ))}
                    {(u.permissoes?.length || 0) > 3 && (
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2.5 bg-blue-900/20 text-blue-400 hover:bg-blue-900/40 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
                >
                  Editar
                </button>
                <button
