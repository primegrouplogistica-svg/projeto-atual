...
const AdminAgregadoManagement: React.FC<AdminAgregadoManagementProps> = ({
  agregados,
  onUpdateAgregados,
  onBack,
}) => {
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Gerencie os parceiros terceirizados e suas placas
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            onClick={onBack}
            className="bg-slate-800 hover:bg-slate-700 px-4 py-2 rounded-lg font-bold text-sm"
          >
            Voltar
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {agregados.map(a => (
          <Card
            key={a.id}
            className={`flex flex-col gap-4 group transition-all border-l-4 ${
              a.ativo ? 'border-l-blue-600' : 'border-l-red-900'
            }`}
          >
            <div className="flex justify-between items-start">
              <div>
                <div className="font-bold text-slate-100 group-hover:text-blue-400 transition-colors text-lg">
                  {a.nome}
                </div>
            ...
            <div className="flex gap-2 mt-auto pt-2 border-t border-slate-800">
              <button
                ...
                className={`flex-1 py-1.5 text-[10px] font-black uppercase tracking-widest rounded transition-colors ${
                  a.ativo
